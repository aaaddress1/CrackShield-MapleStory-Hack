...
void PressKey(HWND handle,DWORD Key){
	PostMessage(handle,WM_KEYDOWN,Key,MakeKeyLParam(Key,WM_KEYDOWN));
	PostMessage(handle,WM_KEYUP,Key,MakeKeyLParam(Key,WM_KEYUP));
}
...
try{
	if(*(long*)0x0E1CD58 == 0)return;
	long ShowTextCall= 0x0867fc2; //8b 0d ? ? ? ? 50 e8 ? ? ? ? c9 c3
	__asm{
		Push 0
		Push 0
		Push 0
